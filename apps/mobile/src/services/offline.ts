...
export async function cacheChapters(chapters: any[]) {
  const x = await db();
  await x.withTransactionAsync(async () => {
...
export async function queueOffline(kind: string, payload: unknown) {
  const x = await db();
  const now = new Date().toISOString();
  await x.runAsync(
    "INSERT OR REPLACE INTO sync_queue VALUES(?,?,?,?)",
    `${kind}:${Date.now()}`,
    kind,
...
export async function pendingSync() {
  const x = await db();
  return x.getAllAsync<{
    id: string;
    kind: string;
    payload: string;
    updated_at: string;
  }>("SELECT * FROM sync_queue ORDER BY updated_at");
}
